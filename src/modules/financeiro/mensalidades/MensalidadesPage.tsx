...
export default function MensalidadesPage() {
  ...
  const buildWhatsAppURL = (member: MembershipItem) => {
    if (!member.phone) return '';
    const phone = member.phone.replace(/\D/g, '');
    const fullPhone = phone.startsWith('55') ? phone : `55${phone}`;
    const [year, month] = (monthFilter || getMonthKey(new Date())).split('-');
    const monthName = monthNames[parseInt(month, 10) - 1] || month;
    const msg = encodeURIComponent(
      `Olá ${member.name}! 🙏\n\n` +
      `Passando para lembrar que a mensalidade de *${monthName}/${year}* do Templo de Umbanda Luz e Fé no valor de *R$ ${formatBRL(member.value)}* está pendente.\n\n` +
          ...
          <div className="mt-4 flex flex-col gap-3">
            <div className="rounded-2xl border border-ink-100 bg-ink-50/50 p-4">
              ...
              <div className="flex flex-col gap-2 md:flex-row md:items-center">
                ...
                />
                <input
                  className="w-full rounded-xl border border-ink-100 bg-white px-3 py-2 text-sm text-ink-700 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100 md:w-44"
                  placeholder="Telefone (DDD+Nº)"
              ...
              <div key={member.id} className="rounded-2xl border border-ink-100 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {member.status === 'pendente' && member.phone && (
                      <a
                        href={buildWhatsAppURL(member)}
                        target="_blank"
                        rel="noopener noreferrer"
                        title={`Cobrar ${member.name} via WhatsApp`}
                        className="flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 transition hover:border-emerald-400 hover:bg-emerald-100"
                      >
                    ...
                    {member.status === 'pendente' && !member.phone && (
                      <span className="text-[10px] text-ink-300" title="Cadastre o telefone para cobrar via WhatsApp">📱 sem tel.</span>
                    )}
                    <button
